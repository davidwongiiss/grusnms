package com.grus.nms.deamon.monitor.xml;
...
public class QAMXml implements Runnable {
	 ...
	 * "<?xml version='1.0'?><NSG><IPINPCFG Action='GET'/></NSG>"; String url =
	 * "http://192.168.11.45/XmlConfig";
	 * 
	 * //new QAMXml("192.168.11.45").saveXml(data, url, "192.168.11.45",
	 * "8ced02b04f8653f76036eef04c1e438940901f2f", "gbe"); Nodes n = new Nodes();
	 * n.setId("e15117e2-4842-11e3-8317-c3f7b0cf833f"); n.setIp("192.168.11.45");
	 * String qam9Data =
	 * "<?xml version='1.0'?><NSG><BLADETRAFFIC Action='GET' Blade='9'/></NSG>";
	 * String url = "http://"+n.getIp()+"/XmlConfig"; new
	 * QAMXml(null).saveXml(qam9Data
	 * ,url,n,"b644ae0720178c3fa1b5db18ca2d0c8c76db196a","qam9"); }
	 */

	public void saveXml(String data, String url, Node node, String session,
			String dataType) {
		/*
		...
		try {
			...
			if (code == HttpStatus.SC_OK) {
				if ("gbe".equals(dataType)) {
					...
					GbeValue gbe = XmlParser.gbeXmlParser(info, node);

					// 将取得到的数据保存到数据库
					ServicesFactory.getIGbeValuesServicesInstance()
							.insert(gbe);
				}
				if ("qam1".equals(dataType)) {
					info = post.getResponseBodyAsString();
					QamValue qam = XmlParser.qamXmlParser(info, node, 1);
					ServicesFactory.getIQamValuesServicesInstance()
							.insert(qam);
				}
				if ("qam2".equals(dataType)) {
					info = post.getResponseBodyAsString();
					QamValue qam = XmlParser.qamXmlParser(info, node, 2);
					ServicesFactory.getIQamValuesServicesInstance()
							.insert(qam);
				}
				if ("qam3".equals(dataType)) {
					info = post.getResponseBodyAsString();
					QamValue qam = XmlParser.qamXmlParser(info, node, 3);
					ServicesFactory.getIQamValuesServicesInstance()
							.insert(qam);
				}
				if ("qam4".equals(dataType)) {
					info = post.getResponseBodyAsString();
					QamValue qam = XmlParser.qamXmlParser(info, node, 4);
					ServicesFactory.getIQamValuesServicesInstance()
							.insert(qam);
				}
				if ("qam5".equals(dataType)) {
					info = post.getResponseBodyAsString();
					QamValue qam = XmlParser.qamXmlParser(info, node, 5);
					ServicesFactory.getIQamValuesServicesInstance()
							.insert(qam);
				}
				if ("qam6".equals(dataType)) {
					info = post.getResponseBodyAsString();
					QamValue qam = XmlParser.qamXmlParser(info, node, 6);
					ServicesFactory.getIQamValuesServicesInstance()
							.insert(qam);
				}
				if ("qam7".equals(dataType)) {
					info = post.getResponseBodyAsString();
					QamValue qam = XmlParser.qamXmlParser(info, node, 7);
					ServicesFactory.getIQamValuesServicesInstance()
							.insert(qam);
				}
				if ("qam8".equals(dataType)) {
					info = post.getResponseBodyAsString();
					QamValue qam = XmlParser.qamXmlParser(info, node, 8);
					ServicesFactory.getIQamValuesServicesInstance()
							.insert(qam);
				}
				if ("qam9".equals(dataType)) {
					info = post.getResponseBodyAsString();
					QamValue qam = XmlParser.qamXmlParser(info, node, 9);
					ServicesFactory.getIQamValuesServicesInstance()
							.insert(qam);
				}
			}
